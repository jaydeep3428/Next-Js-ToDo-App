...
export default function Home() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/tasklists");
  ...
  const addOrUpdateTask = async () => {
    ...
    const url = isEditing
      ? `http://localhost:3000/api/tasklists/${tasks[currentTaskIndex]._id}`
      : "http://localhost:3000/api/tasklists";
  ...
  const removeTask = async (index) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/tasklists/${taskId}`,
        {
          method: "DELETE",
        }
      );
